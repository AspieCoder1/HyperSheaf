...
@pytest.fixture
def sheaf_learner(hypergraph: HeteroHypergraph) -> TypeEnsembleSheafLearner:
    return TypeEnsembleSheafLearner(
        node_feats=16,
        out_channels=32,
        act_fn="relu",
...
def test_type_ensemble_sheaf_learner(
    sheaf_learner: TypeEnsembleSheafLearner, sheaf_learner_input: SheafLearnerInput
):
    assert str(sheaf_learner) == "TypeEnsembleSheafLearner()"
...
def test_type_ensemble_sheaf_learner_predict_sheaf(
    sheaf_learner: TypeEnsembleSheafLearner, sheaf_learner_input: SheafLearnerInput
):
    out1 = sheaf_learner(
        sheaf_learner_input.node_feats,
...
def test_type_ensemble_sheaf_learner_act_fn(
